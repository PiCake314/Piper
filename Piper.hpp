...
// requires std::invocable<D, Types...>
struct Package{
    const D* derived;
...
public:
    ...
    [[nodiscard]] constexpr Package<Derived, Types...> operator()(Types... params) const {
...
struct Composed : Piper<Composed<P1, P2>>{
    ...
    template <typename... Types>
    constexpr auto operator()(Types&&... v) const { // regular syntax
        return p2(p1(std::forward<Types>(v)...));
    }

    [[nodiscard]] constexpr auto operator()() const { // returns a package to invoke |
